...
The first round of games is generated automatically when a session starts. After each round of play, select \*\*New Game\*\* to generate the next round.
...
When the current round has finished, select \*\*New Game\*\*.
...
Each time a new round is generated, CourtZ attempts to produce the best possible set of games using the current session settings.



Depending on those settings, CourtZ may consider factors such as:
...
The aim is to produce enjoyable, balanced games while giving everyone a fair opportunity to play.
...
If there are more players than available court spaces, some players will remain in the Waiting Room until they can be included in a later round.
...
If you start a session without using a Session Profile, or you've adjusted the current session to create a setup you'd like to use again, you can save it as a Session Profile.



Select \*\*Options\*\* (⋮) and choose \*\*Save Session Profile\*\*.
